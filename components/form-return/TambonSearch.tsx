  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      try {
        ...
        const safeResults = Array.isArray(filteredData) ? filteredData : [];
    ...
    <div className={`relative tambon-search-container ${className}`}>
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-10 py-2 text-sm text-gray-900 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-green-400 transition-all"
        />
            ...
            <ul className="py-1">
              {results.map((item, index) => (
                <li
                  ...
                  className="cursor-pointer hover:bg-green-50 px-4 py-3 border-b border-gray-100 last:border-b-0 transition-colors"
                >
                  <div className="flex items-start">
                    <svg className="h-4 w-4 text-green-500 mr-3 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
